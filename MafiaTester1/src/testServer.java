 ...
 *      타이머를 짜놓은 함수는 깃허브에 TimeTestFunc라고 업로드해두었음.   
 * */
...
* @ writers[PrintWriter//HashSet]   : hashset about writer????????????????????? 어떤 기능을 하는 변수인지.... -> 유저의 주소를 받는 HashSet(중복 허용 x)
   ...
   private static class Handler extends Thread {
      ...
      public void run() {
         try {
            ...
            while (true) {//read a name of player
               ...
               synchronized (names) {
                  ...
                  if (!names.contains(name)) { //there must be no duplicate in name.
                     ...
                     for (PrintWriter writer : writers) { //if any user enters, memo in server's console. 맞나..??? -> 새로운 유저가 들어올 경우 채팅창으로 유저들에게 [name] enter라고 알려줌
            ...
            out.println("NAMEACCEPTED"); //if user enters own name, give check-message. ??? ->클라이언트에게 NAMEACCPETED라는 프로토콜메세지를 보내줌 ->유저의 채팅창 활성화 시켜줌
            ...
            if (client_count == max_client) {
               ...
               //tell a job to each players.? 이 과정이 맞나..?--직업배정받은 정보를 소켓에 담아 클라이언트에게 주면 팝업창으로 알려주는 게 좋을 것 같음... 
               for (int i = 0; i < max_client; i++)
            ...
            while (true) {
               ...
               //이름??? 뭐하는 부분인지 잘 모르겠어요...
               // 11월24일 모임때 말씀하셨던 투표를 할때 서버에서 클라이언트들의 이름을 받아오기 위한 과정입니다. 유저가 채팅창에 /name이라고 치면 현재 들어와 있는 유저들의 이름이 gui팝업창으로 뜸
               // 아직 미완성
