 ...
 * 서버가 할 일
 * 
 * 1. 현재 코드 정리하기, 함수로 써주시면 깔끔해질 것 같음.
...
* @ writers[PrintWriter//HashSet]	: hashset about writer????????????????????? 어떤 기능을 하는 변수인지....
...
public class testServer{
   ...
   private static class Handler extends Thread {
      ...
      public void run() {
            ...
            while (true) {//read a name of player
               ...
               synchronized (names) {
                  ...
                  if (!names.contains(name)) { //there must be no duplicate in name.
                     ...
                     for (PrintWriter writer : writers) { //if any user enters, memo in server's console. 맞나..???
         ...
         } catch (IOException e) { //갑자기 catch가 나온 이유..?
