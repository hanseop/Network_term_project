...
public class testClient {
	 ...
	 * contents to the server. Note howe127.0.0.1ver that the textfield is initially
	...
	public testClient() {
		...
		textField.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				out.println(textField.getText());// 메세지 보내는 부분
				textField.setText("");// 버퍼 비워줌
			}
		});
	}
	...
	private void run() throws IOException {
		...
		String serverAddress = getServerAddress(); // serveraddress : ip address
		...
		while (true) {
			...
			if (line.startsWith("SUBMITNAME")) {
				out.println(getName()); // input name
